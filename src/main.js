...
function startGame(slotId = null, loadPlayerData = null, charName = null) {
    ...
    network.onRemoteLootSpawn = (data) => {
        // data: { id, baseId, name, rarity, icon, x, y, ownerId }
        const myName = player?.charName || 'guest';
        if (data.ownerId && data.ownerId !== myName) return;
...
function gameLoop(timestamp) {
            ...
            if (network.isHost) {
                ...
                if (now - window._lastEnemySyncTime > 100 && enemies.length > 0) {
                    ...
                    const enemyData = enemies.filter(e => e.hp > 0).map(e => ({
                        ...
                        anim: e.animState,
                        dir: e.facingDir,
                        type: e.type, // e.g. 'skeleton' or 'boss'
                        name: e.name,
                        icon: e.icon,
                        isBoss: e.isBoss || e.type === 'boss'
                    }));
    ...
    // Dropped items (with loot filter and owner check)
    for (const di of droppedItems) {
        if (di.ownerId && di.ownerId !== player.charName) continue; // Instanced Loot check
...
function checkDeaths() {
    ...
    for (const e of enemies) {
        if (e.hp <= 0 && e.state !== 'dead') {
            ...
            if (network.isHost) {
                const zonePlayers = [player, ...network.otherPlayers.values()];
                const myName = player?.charName || 'guest';

                for (const p of zonePlayers) {
                    const charName = p.charName || p.name || (p === player ? myName : 'guest');

                    // Bosses guarantee a loot explosion (Multi-drop) per player
                    if (e.type === 'boss') {
                        ...
                        for (let i = 0; i < dropCount; i++) {
                            ...
                            if (bossItem) {
                                ...
                                bossItem.ownerId = charName;
                                droppedItems.push(bossItem);
                                network.broadcastLootSpawn(bossItem, charName);
                    ...
                    const pMf = p.magicFind !== undefined ? p.magicFind : (p.mf || 0);
                    const item = loot.roll(e, { magicFind: pMf });
                    if (item) {
                        item.x = e.x + (Math.random() - 0.5) * 20;
                        item.y = e.y + (Math.random() - 0.5) * 20;
                        item.id = `loot_${Date.now()}_${Math.random()}`;
                        item.ownerId = charName;
                        droppedItems.push(item);
                        network.broadcastLootSpawn(item, charName);
                    ...
                    // Gold Drops
                    const pGf = p.goldFind !== undefined ? p.goldFind : (p.gf || 0);
                    const goldAmt = loot.rollGold(e, pGf);
                    if (goldAmt > 0) {
                        const goldObj = {
                        ...
                        droppedGold.push(goldObj);
                        network.broadcastGoldSpawn(goldObj, charName);
                    }
                }
            }
...
function getIconForSkill(id) {
    const iconMap = {
        ...
        'arms': 'ra-sword',
        'bash': 'ra-muscle-up',
        'double_swing': 'ra-crossed-swords',
        ...
        'spell_reflection': 'ra-circular-shield',
        'vanguard': 'ra-heavy-shield',
        'second_wind': 'ra-player-pain',
        ...
        'nova': 'ra-explosion',
        'energy_shield': 'ra-circular-shield',
        ...
        'holy_mastery': 'ra-sun',
        'divine_shield': 'ra-circular-shield',
        'divine_storm': 'ra-water-drop',
        ...
        'avengers_shield': 'ra-circular-shield',
        ...
        'rain_of_chaos': 'ra-fire-bomb',
        'rain_of_fire': 'ra-fire-bomb',
        'hellfire': 'ra-fire',
        ...
        'wolf_companion': 'ra-wolf-howl',
        'spirit_guide': 'ra-aware',
        'harmony': 'ra-radial-balance',
        ...
        // ========== PALADIN NODES ==========
        'seal_of_righteousness': 'ra-dripping-sword',
        'crusader_mastery': 'ra-book',
        'crusader_strike': 'ra-lightning-sword',
        'judgement': 'ra-gavel',
        'hammer_of_wrath': 'ra-hammer-drop',
        'holy_shield': 'ra-circular-shield',
        'prot_mastery': 'ra-book',
        'hammer_righteous': 'ra-hammer-drop',
        'shield_of_righteousness': 'ra-circular-shield',
        'holy_wrath': 'ra-sunbeams',
        'guardian_of_ancient_kings': 'ra-angel-wings',
        'blessing_of_kings': 'ra-crown',
        'lay_on_hands': 'ra-health',
        ...
        // ========== WARLOCK NODES ==========
        'malefic_grasp': 'ra-tentacle',
        'soul_siphon': 'ra-glass-heart',
        'ember_storm': 'ra-burning-embers',
        'immolate_warlock': 'ra-fire-bomb',
        'master_demonologist': 'ra-dragon',
        'demonic_sacrifice': 'ra-skull',
        'demonic_empowerment': 'ra-bolt-shield',
        'backdraft': 'ra-cycle',
        ...
        // ========== RANGER NODES ==========
        'trap_mastery_r': 'ra-caltrop',
        'nature_affinity': 'ra-leaf',
        'comp_mastery': 'ra-animal-skull',
        'tracking': 'ra-aware',
        'mark_death': 'ra-targeted',
        'spirit_bond': 'ra-chain',
        'aspect_hawk': 'ra-eagle-emblem',
        ...
        // ========== TREE HEADERS (Fixing blanks) ==========
        'combat': 'ra-crossed-swords',
        'traps': 'ra-bear-trap',
        'nature': 'ra-leaf',
        'healing': 'ra-health-potion',
        'retribution': 'ra-flaming-trident',
        'protection': 'ra-heavy-shield',
        'arms': 'ra-sword',
        'battle': 'ra-castle-flag',
        'summoning': 'ra-tombstone',
        'bone': 'ra-bone-bite',
        ...
        'ww_radius': 'ra-whirlwind',
        'ww_fortress': 'ra-heavy-shield',
        'cleave_arc': 'ra-split-body',
        'cleave_momentum': 'ra-fast-forward',
        'fb_radius': 'ra-fireball',
        'fb_plasma': 'ra-burning-embers',
        'orb_nova': 'ra-ice-cube',
        'orb_zero': 'ra-snowflake',
        'cl_conductivity': 'ra-lightning-bolt',
        'cl_static': 'ra-electric',
        'lb_fork': 'ra-lightning-storm',
        'lb_overload': 'ra-lightning-trio',
        'st_multi': 'ra-totem',
        'st_mobile': 'ra-incense',
        'eq_tectonic': 'ra-mountains',
        'eq_stone': 'ra-stone-tower',
        'skel_warrior': 'ra-skeleton',
        'skel_grave': 'ra-skull',
        'bs_splinter': 'ra-spear-head',
        'bs_ossified': 'ra-bone-knife',
        'ce_vile': 'ra-poison-cloud',
        'ce_blood': 'ra-blood',
        'ds_kidney': 'ra-daggers',
        'ds_poison': 'ra-dripping-blade',
        'sb_blindside': 'ra-cloudy-smoke',
        'sb_lingering': 'ra-fizzing-flask',
        'smite_holy': 'ra-heavy-fall',
        'smite_stun': 'ra-broken-shield',
        'ha_retribution': 'ra-sun',
        'ha_divinity': 'ra-angel-wings',
        'maul_ravage': 'ra-bear-claw',
        'maul_shaker': 'ra-earth-crack',
        'torn_eye': 'ra-cyclone',
        'torn_arctic': 'ra-ice-shards',
        'sb_siphon': 'ra-shadow-follower',
        'sb_chain': 'ra-chain',
        'cow_enfeeble': 'ra-broken-heart',
        'cow_rot': 'ra-internal-organ',
        'ms_rain': 'ra-arrow-cluster',
        'ms_broadhead': 'ra-broadhead',
        'et_cluster': 'ra-bomb-explosion',
        'et_napalm': 'ra-large-fire',
    };
...
function updateRespawns() {
    ...
    // MMO: Only the Host (or single player) handles respawns to avoid duplicates
    if (network && network.isConnected && !network.isHost) return;
